...
void mkdirectedadjlistOUT(adjlist* g){
	unsigned long i,u,v;
	unsigned long *d=calloc(g->n,sizeof(unsigned long));
...
unsigned long* renameNodes(adjlist* a, adjlist* in){
	unsigned long* exName = malloc(a->n*sizeof(unsigned long));
	unsigned long* newName = malloc(a->n*sizeof(unsigned long));
	unsigned long* newcd = malloc((a->n+1)*sizeof(unsigned long));

	newcd[0]=0;
	unsigned long k = 0;
	for(unsigned long i=0;i<a->n;++i){
		if(a->cd[i+1]-a->cd[i] != 0){
			exName[k] = i;
			newName[i] = k;
	...
	for(unsigned i=0;i<a->e;++i){

		in->adj[i] = newName[in->adj[i]];
			
	}

	newcd = realloc(newcd,(k+1)*sizeof(unsigned long));
	in->cd = newcd;
	in->n=k;
...
void free_renamedadjlist(adjlist *g){
	free(g->edges);
	free(g->adj);
	free(g);
}
